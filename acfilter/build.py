...
if __name__ == "__main__":
    ...
    sample = "我现在保存了一个pkl文件，我想的是这个类用来读取pkl然后返回一个类似bool值/类来提示是否有敏感词"
    matches = ac.find_all(sample) 
    print("Matches:", matches)
    # 替换为星号（注意：当前实现是基于规范化文本的替换）
    masked = ac.replace_with_mask(sample, mask_char="*")
    print("Masked:", masked)
